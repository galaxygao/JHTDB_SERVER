...
> 实施状态（2026-08-30）：2a 全域正负 Π 分拆已完成，输出 `weak_asymmetry.json/html`，提供 `compute-weak-asymmetry` 和只读 GUI 页面，并在常规 Cq 扫描中复用同一批 chunk 同步生成。下述“2a-扩展”按象限细分正负 Π 仍是下一项任务，尚未实现。
